...
const TestEngine = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Redirect to new Result Review page with data
      navigate('/result', { 
        state: { 
          test, 
          answers, 
          timeTaken: totalTimeTaken,
          result: data.result // Pass backend result just in case
        } 
      });
    } catch (err) {
  ...
  // ─── Results View (Moved to /result route) ──────────────────────────────────
        ...
        <div className="question-panel">
          ...
          <div className="nav-buttons">
            ...
            <button
              ...
              onClick={() => goToQuestion((currentQ + 1) % test.questions.length)}
            >Next →</button>
          </div>
        </div>
